/*
 * The purpose of this file is to behave as a library for all Media Most Popular tests
 * These tests should not take the dom into account. This means the tests can be re-used
 */
YUI.add("media-most-popular-library", function(Y) {

    var A = Y.Assert;

    var self = Y.namespace("Media.Test").MediaMostPopular = 
    {
        ...
        validateTextTitle: function(node, nodeTitle)
        {
        	...
        	var actualTitle = node.one("em").get("textContent");
        	A.areEqual(nodeTitle, actualTitle, "Confirm Module Title Matches")
        },
